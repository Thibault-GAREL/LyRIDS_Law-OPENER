...
The two protocols do not exercise the same zero-shot head (\autoref{tab:protocols}). \autoref{tab:legal-gold} measures the label-name geometry on its own, and \autoref{tab:legal-main} the operating point one would deploy.
...
\begin{tablenotes}[flushleft]
\scriptsize
\item Detector mentions, offset/sentinel alignment (\autoref{sec:method:evaluation}). Best per column in bold, within each block, ties included, the two AMI sub-blocks bolded apart since OPENER-Sup is the only row that consumes target labels. OPENER rows are mean\,$\pm$\,std over three retraining seeds, baselines are frozen checkpoints with a single deterministic run. \rev{Model size and cost in \autoref{tab:legal-efficiency}, per-corpus cost in \autoref{tab:legal-cost-detail}.}
...
\rev{Detection collapses on German, but is that the language or the schema?} GLiNER-L recovers $43.9\%$ of the Portuguese mentions, on a par with the $44.5\%$ it reaches on English Indian judgements, \rev{against $18.7\%$ on German.} \rev{Yet German-LER is also the corpus of long citations}, since legal-norm and case-law citations make up $62\%$ of its mentions. \rev{The boundary analysis below separates the two.}

\rev{\textbf{Where end-to-end quality is actually lost.} \autoref{tab:legal-decomp} splits each end-to-end score into its two stages, the rows of a pair differing \emph{only} in the typing head. The supervised head is then worth more than its $+1.0$ AMI suggests. On correctly detected mentions, it types $78.7\%$ right, against the detector's own $64.6\%$. That $14$-point gain is a mean over corpora lifted by German. Pooled over mentions it is $81.3$ against $75.1$. The end-to-end average hides it, because it applies only to the $35.7\%$ of mentions that survive detection. The zero-shot head moves the other way, $64.5\%$ against $70.1\%$, which is why its fusion sits below the detector it fuses with. Raising that share to $100\%$, which is what the oracle column reports, lifts the end-to-end score from $35.9$ to ${\approx}70$ AMI and not to $100$, the two being different scales. What remains are the mentions the detector never proposed, which fall into a single sentinel class whatever their gold type, and that bound is itself a generous one (\autoref{tab:legal-decomp}).}
...
\toprule
System & \multicolumn{2}{c}{\rev{Detect.\ (\% gold)}} & \rev{Type} & \multicolumn{3}{c}{\rev{End-to-end (${\times}100$)}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-4}\cmidrule(lr){5-7}
 & exact & touched & \rev{acc.$^\dagger$} & F1 & AMI & oracle \\
\midrule
GLiNER-S & $34.4$ & $55.1$ & $62.2$ & $22.2$ & $33.0$ & $65.9$ \\
GNER-T5 & $39.8$ & $77.2$ & $56.5$ & $18.4$ & $32.4$ & $74.2$ \\
Qwen-1.5B$^\ddagger$ & $14.2$ & $20.5$ & \rev{$(68.2)^\P$} & $10.4$ & $23.1$ & $42.2$ \\
...
\begin{tablenotes}[flushleft]
\scriptsize
\item[$\dagger$] \rev{The first two columns are shares of the \emph{gold} mentions, whereas \emph{acc.} is a share of the mentions \emph{already detected}, so their denominators differ. All columns are unweighted means over the four corpora.}
\item[$\P$] \rev{Not comparable, from $343$ exact detections only, $4$ on German (all typed right). Pooled over mentions, $56.9$.}
\item \rev{\emph{exact}: gold mentions recovered on both offsets, which is the recall of \autoref{sec:exp:results}. \emph{touched}: also counting mentions a prediction overlaps without matching their boundaries, so the gap to \emph{exact} is made of boundary errors. \emph{acc.}: share of the exactly detected mentions given the right type, which isolates the typing head from detection. \emph{F1}: micro entity-level, offsets and type both correct. \emph{oracle}: the ceiling these detected spans allow, each correctly detected span taking its gold type and every spurious span a single shared label. It is a function of the detector alone, hence identical within a pair, and it is a \emph{loose} upper bound, since recognising a span as spurious is detection rather than typing. Every figure here is the released seed alone, so the AMI column can differ slightly from the three-seed means of \autoref{tab:legal-main} ($33.7$ against $33.4$ for OPENER-ZS, for instance).}
...
\rev{\textbf{Many detection failures are boundaries, not blindness.} Counting a mention as \emph{touched} when a prediction overlaps it without matching both offsets, GLiNER-L touches $68.6\%$ of gold mentions but aligns only $41.1\%$ exactly, and the generative baseline is the extreme case at $77.2\%$ against $39.8\%$ (\autoref{tab:legal-decomp}). German is the one corpus this does not rescue, GLiNER-L reaching $38.4\%$ there even relaxed. Long spans are not the cause. GLiNER-L touches $94\%$ of Portuguese legislation and $95\%$ of Indian precedents, both long citations, but only $42\%$ of German persons, at $1.2$ words on average, against $79$ to $100\%$ for persons elsewhere. German fails as a language, not as a schema. An in-language \emph{detector} would therefore lift the ceiling far more than a better typing head.}

\rev{\textbf{What the two heads confuse.} The confusions are exactly the ones the design predicts (\autoref{sec:appendix:errors}). On German-LER, OPENER-ZS maps \textsc{norm} onto \textsc{regulation} $213$ times, its dominant error by far, where the probe makes that same confusion $3$ times. The two anchor names are near-synonyms in English, while the corpus separates the types sharply. On Indian-Legal it collapses \textsc{other-person} into procedural roles $123$ times, which no label-name prototype can avoid, since those types all name people. Supervision, not a better embedder, is what the label-free head lacks.}
...
\begin{tablenotes}[flushleft]
\scriptsize
\item End-to-end. \rev{Only AMI is a three-seed mean\,$\pm$\,std. F1, latency and energy come from the released seed-$42$ run alone.} Energy per $1000$ evaluated sentences (multiply by $0.056$ for gCO\textsubscript{2}). Best per column in bold, within each sub-block. Params count the whole pipeline, detector plus the $137$M embedder, so OPENER-Sup is lighter than OPENER-ZS only because it runs on GLiNER-M rather than GLiNER-L. \rev{F1 is the micro entity-level score of \autoref{tab:legal-decomp}.}
