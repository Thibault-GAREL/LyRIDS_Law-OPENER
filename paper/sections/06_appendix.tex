\section{\rev{Datasets}}
\label{sec:appendix:datasets}

\rev{\autoref{tab:legal-datasets} gives the size and the published supervised topline of each corpus, and \autoref{tab:legal-distributions} the share of test mentions taken by each entity type.}

\begin{table}[H]
\centering
\caption{Legal NER datasets\rev{, with the published supervised topline of each}.}
\label{tab:legal-datasets}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{threeparttable}
\begin{tabular}{@{}llrrcc@{}}
\toprule
Dataset & Lang & Sent. & Ment. & Types & \rev{Sup. F1$^\dagger$} \\
\midrule
E-NER & en & 877 & 1{,}793 & $7$ & \rev{$96.1$} \\
Indian-Legal & en & 866 & 3{,}028 & $14$ & \rev{$91.1$} \\
LeNER-Br & pt & 636 & 1{,}536 & $6$ & \rev{$92.5$} \\
German-LER & de & 1{,}000$^\P$ & 1{,}740 & $7$ & \rev{$96.0$} \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\scriptsize
\item Test sentences carrying $\geq 1$ entity, and the mentions actually evaluated. Sentences with no gold entity are evaluated by no system. German-LER is shown with its 7-type \emph{coarse} schema (19-type \emph{fine} schema in \autoref{sec:exp:analysis}).
\item[$\P$] Of $3{,}018$, the only split above our $1000$-sentence cap, every score for it being computed on those first $1000$.
\item[$\dagger$] \rev{Best entity-level F1 reported with the corpus itself \citep{au2022ener,kalamkar2022indianlegal,luzdearaujo2018lenerbr,leitner2020germanler}, by a tagger trained in-domain that learns detection jointly with typing (cross-validated on E-NER and German-LER, held-out test split on the other two). Not a competitor to a pipeline that retrains nothing, quoted as the scale of the task.}
\end{tablenotes}
\end{threeparttable}
\end{table}

\begin{table}[H]
\centering
\caption{\rev{Entity types and their share of test mentions.}}
\label{tab:legal-distributions}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{threeparttable}
\rev{\begin{tabular}{@{}p{0.20\linewidth}p{0.72\linewidth}@{}}
\toprule
Dataset & Entity types (share \% of test mentions) \\
\midrule
E-NER (en) & Business 41.8, Misc 18.3, Legislation/Act 13.5, Location 11.3, Person 7.6, Government 7.1, Court 0.3 \\
\addlinespace[2pt]
Indian-Legal (en) & Lawyer 16.7, Court 9.3, Respondent 8.8, Other-person 8.7, Provision 8.5, Statute 7.3, Date 7.1, Petitioner 6.0, then GPE, Precedent, Org, Judge, Case-number and Witness, from 5.9 down to 1.9 \\
\addlinespace[2pt]
LeNER-Br (pt) & Organiza\c{c}\~{a}o 32.6, Legisla\c{c}\~{a}o 24.6, Pessoa 15.2, Tempo 12.5, Jurisprud\^{e}ncia 12.0, Local 3.1 \\
\addlinespace[2pt]
German-LER (de) & Legal-norm 38.3, Case-law 23.4, Organization 15.0, Regulation 6.7, Person 6.1, Legal-literature 5.9, Location 4.7 \\
\bottomrule
\end{tabular}}
\begin{tablenotes}[flushleft]
\scriptsize
\item \rev{German-LER in its 7-type coarse schema. Legal-norm and case-law together account for $61.7\%$ of its mentions, and both are long multi-token citations, which is what makes exact-offset detection hardest on that corpus.}
\end{tablenotes}
\end{threeparttable}
\end{table}

\section{\rev{Error analysis}}
\label{sec:appendix:errors}

\rev{\autoref{tab:confusions} lists, for each corpus, the typing confusions each head makes most often over the mentions its detector recovered exactly, and \autoref{tab:examples} shows real mentions on which at least one head errs. Both are computed on the released seed.}

\rev{Three patterns run through them. Types whose \emph{names} are near-synonyms defeat the label-free head and not the probe, German \textsc{nrm} against \textsc{reg} being the extreme case ($213$ against $3$). Types that share a surface form and differ only by procedural role, as the Indian person types do, defeat prototypes by construction, since the anchor words cannot encode a role. And the probe is not uniformly better. On LeNER-Br it sends \textsc{organizacao} to \textsc{local} as often as the label-free head does, $15\%$ of the detected organisations for both ($41$ against $36$ in raw counts).}

\begin{table}[H]
\centering
\caption{\rev{Most frequent typing confusions (gold\,$\rightarrow$\,predicted), on exactly detected mentions.}}
\label{tab:confusions}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{threeparttable}
\rev{\begin{tabular}{@{}llr@{\hskip 8pt}lr@{}}
\toprule
 & \multicolumn{2}{c}{\cellcolor{hlyellow}OPENER-ZS} & \multicolumn{2}{c}{\cellcolor{hlred}OPENER-Sup} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Corpus & confusion & $n$ & confusion & $n$ \\
\midrule
E-NER & \textsc{gov}\,$\rightarrow$\,\textsc{bus} & $65$ & \textsc{bus}\,$\rightarrow$\,\textsc{misc} & $18$ \\
      & \textsc{bus}\,$\rightarrow$\,\textsc{misc} & $29$ & \textsc{misc}\,$\rightarrow$\,\textsc{leg} & $10$ \\
\addlinespace[2pt]
Indian & \textsc{o-per}\,$\rightarrow$\,\textsc{petit} & $66$ & \textsc{resp}\,$\rightarrow$\,\textsc{prec} & $50$ \\
       & \textsc{lawyer}\,$\rightarrow$\,\textsc{prec} & $60$ & \textsc{petit}\,$\rightarrow$\,\textsc{prec} & $20$ \\
       & \textsc{o-per}\,$\rightarrow$\,\textsc{witn} & $38$ & \textsc{petit}\,$\rightarrow$\,\textsc{o-per} & $20$ \\
\addlinespace[2pt]
LeNER-Br & \textsc{org}\,$\rightarrow$\,\textsc{local} & $36$ & \textsc{org}\,$\rightarrow$\,\textsc{local} & $41$ \\
         & \textsc{tempo}\,$\rightarrow$\,\textsc{local} & $22$ & \textsc{org}\,$\rightarrow$\,\textsc{legis} & $15$ \\
\addlinespace[2pt]
German & \textsc{nrm}\,$\rightarrow$\,\textsc{reg} & $\mathbf{213}$ & \textsc{nrm}\,$\rightarrow$\,\textsc{rs} & $9$ \\
       & \textsc{per}\,$\rightarrow$\,\textsc{org} & $12$ & \textsc{reg}\,$\rightarrow$\,\textsc{nrm} & $6$ \\
\bottomrule
\end{tabular}}
\begin{tablenotes}[flushleft]
\scriptsize
\item \rev{Raw counts, over different denominators. The heads run on different detectors (GLiNER-L for ZS, GLiNER-M for Sup), so they type different numbers of exactly detected mentions, $326$ against $177$ on German. As a share of the detected gold type, \textsc{nrm}\,$\rightarrow$\,\textsc{reg} is $96\%$ against $3.5\%$, and LeNER-Br \textsc{org}\,$\rightarrow$\,\textsc{local} $15\%$ for both.}
\item \rev{Type codes abbreviated. German: \textsc{nrm} legal norm, \textsc{reg} regulation, \textsc{rs} case law. Indian: \textsc{o-per} other-person, \textsc{petit} petitioner, \textsc{resp} respondent, \textsc{prec} precedent, \textsc{witn} witness.}
\end{tablenotes}
\end{threeparttable}
\end{table}

\begin{table}[H]
\centering
\caption{\rev{Real mentions on which at least one typing head errs.}}
\label{tab:examples}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{threeparttable}
\rev{\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.30\linewidth}lll@{}}
\toprule
Mention & Gold & \cellcolor{hlyellow}ZS & \cellcolor{hlred}Sup \\
\midrule
\multicolumn{4}{@{}l}{\emph{E-NER (en)}} \\
\texttt{NXK}, \texttt{JSN}, \texttt{JGV} & \textsc{bus} & \textsc{misc} & \textsc{bus} \\
\texttt{Internet} & \textsc{misc} & \textsc{gov} & \textsc{misc} \\
\addlinespace[2pt]
\multicolumn{4}{@{}l}{\emph{LeNER-Br (pt)}} \\
\texttt{Superior Tribunal Militar} & \textsc{org} & \textsc{jurisp} & \textsc{org} \\
\texttt{Uni\~{a}o} & \textsc{org} & \textsc{local} & \textsc{org} \\
\addlinespace[2pt]
\multicolumn{4}{@{}l}{\emph{German-LER (de)}} \\
\texttt{\S\,63 Abs.\,1 AO} & \textsc{nrm} & \textsc{reg} & \textsc{nrm} \\
\texttt{\S\,130a VwGO} & \textsc{nrm} & \textsc{reg} & \textsc{nrm} \\
\texttt{35. BImSchV} & \textsc{nrm} & \textsc{reg} & \textsc{reg} \\
\addlinespace[2pt]
\multicolumn{4}{@{}l}{\emph{Indian-Legal (en)}} \\
\texttt{Najmi Waziri} & \textsc{judge} & \textsc{prec} & \textsc{prec} \\
\texttt{State Of Gujarat} & \textsc{resp} & \textsc{gpe} & \textsc{prec} \\
\bottomrule
\end{tabular}}
\begin{tablenotes}[flushleft]
\scriptsize
\item \rev{The opaque E-NER tickers carry no lexical signal a label name could match, so the zero-shot head defaults to \textsc{misc} while the probe recovers them from the distribution. The last German row and the two Indian rows are cases both heads get wrong. Each row is one occurrence. Typing is contextual (\autoref{sec:method:preprocessing}), so a recurring mention can be typed differently across sentences. The zero-shot head types \texttt{Uni\~{a}o} right in $3$ of its $4$ detected occurrences, and \texttt{State Of Gujarat} in $2$ of $3$.}
\end{tablenotes}
\end{threeparttable}
\end{table}

\section{\rev{The two protocols side by side}}
\label{sec:appendix:protocols}

\begin{table}[H]
\centering
\caption{\rev{What differs between the two protocols. Stages one and two are shared, only the typing head changes.}}
\label{tab:protocols}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{threeparttable}
\rev{\begin{tabular}{@{}p{0.30\linewidth}p{0.30\linewidth}p{0.30\linewidth}@{}}
\toprule
 & Typing on gold & End-to-end \\
\midrule
Mentions typed & gold spans & detector spans \\
ZS prototypes & bare anchor words & anchors in $4$ templates \\
Transductive pass & no & yes ($3$ iterations) \\
Detector fusion & no detector in the loop & yes ($\beta{=}0.05$) \\
Sup probe fit on & gold train mentions & detected train mentions \\
Reported in & \autoref{tab:legal-gold} & \autoref{tab:legal-main} \\
\bottomrule
\end{tabular}}
\begin{tablenotes}[flushleft]
\scriptsize
\item \rev{Typing on gold isolates the typing head, so the zero-shot side reduces to its first step: with no detector there is no label to fuse, and re-estimating prototypes on a gold mention set would measure something the deployed system never sees.}
\end{tablenotes}
\end{threeparttable}
\end{table}

\section{\rev{Fusion weight sweep}}
\label{sec:appendix:beta}

\rev{The weight $\beta$ of Equation~(3) balances the label-name centroids against the detector's own labels. It is fixed a priori at $0.05$, and sweeping it post-hoc on the released model is a pure inference knob that leaves the embedder untouched. At $\beta{=}0$ the refined prototypes alone reach $32.4$ average AMI ($30.5$ without the transductive pass), and as $\beta$ grows the head interpolates up to the GLiNER-L labels ($35.0$), a plateau reached by $\beta{\approx}0.2$. The label-free head can therefore do no better end-to-end than the detector it fuses with.}

\rev{The sweep also exposes a component that does not survive the transfer. At $\beta{=}0.05$, dropping the transductive pass would \emph{raise} the average from $33.7$ to $34.8$ (released seed, hence the offset from the $33.4$ of \autoref{tab:legal-main}). Re-estimating prototypes on test mentions helps when those are mostly true entities, as they are in our general-domain runs \citep{garel2026opener}, whereas here two thirds of the detected spans are alignment noise and the centroids drift towards it. We keep the released configuration rather than tune it on legal test sets.}

\section{\rev{Cost per corpus}}
\label{sec:appendix:cost}

\rev{\autoref{tab:legal-cost-detail} breaks the two cost axes down per corpus. \autoref{tab:legal-efficiency} reports their four-set means, which is what the discussion uses, the per-corpus spread being driven by sentence length rather than by the system.}

\begin{table}[H]
\centering
\caption{\rev{End-to-end latency and energy, per corpus.}}
\label{tab:legal-cost-detail}
\scriptsize
\setlength{\tabcolsep}{1.5pt}
\begin{threeparttable}
\begin{tabular}{@{}lccccc@{}}
\toprule
Model & E-NER & Indian & LeNER-Br & German & Avg \\
      & (en)  & (en)   & (pt)     & (de)   &     \\
\midrule
\multicolumn{6}{l}{\emph{Latency}, median per sentence (ms, $\downarrow$)} \\
GLiNER-S & $\mathbf{20}$ & $\mathbf{22}$ & $\mathbf{24}$ & $\mathbf{22}$ & $\mathbf{22}$ \\
GLiNER-M & $36$ & $35$ & $37$ & $39$ & $37$ \\
GLiNER-L & $65$ & $88$ & $92$ & $78$ & $81$ \\
GNER-T5  & $2897$ & $15784$ & $8768$ & $5705$ & $8289$ \\
Qwen-1.5B$^\ddagger$ & $4852$ & $8068$ & $5929$ & $2857$ & $5426$ \\
OWNER$^\S$ & $328$ & $755$ & $420$ & $398$ & $475$ \\
\rowcolor{hlyellow} OPENER-ZS & $104$ & $140$ & $172$ & $115$ & $133$ \\
\rowcolor{hlred} OPENER-Sup$^\ast$ & $78$ & $102$ & $85$ & $73$ & $84$ \\
\addlinespace
\multicolumn{6}{l}{\emph{Energy} per $1000$ evaluated sentences (Wh, $\downarrow$)} \\
GLiNER-S & $\mathbf{0.4}$ & $\mathbf{0.5}$ & $\mathbf{0.5}$ & $\mathbf{0.5}$ & $\mathbf{0.5}$ \\
GLiNER-M & $0.7$ & $0.8$ & $0.8$ & $0.7$ & $0.7$ \\
GLiNER-L & $1.3$ & $1.7$ & $1.8$ & $1.6$ & $1.6$ \\
GNER-T5  & $49.2$ & $161.0$ & $106.3$ & $67.3$ & $96.0$ \\
Qwen-1.5B$^\ddagger$ & $80.5$ & $157.7$ & $(2886)^\P$ & $63.3$ & $100.5$ \\
OWNER$^\S$ & $10.0$ & $23.1$ & $12.9$ & $12.2$ & $14.6$ \\
\rowcolor{hlyellow} OPENER-ZS & $2.1$ & $3.3$ & $3.4$ & $2.1$ & $2.7$ \\
\rowcolor{hlred} OPENER-Sup$^\ast$ & $1.4$ & $2.4$ & $1.6$ & $1.3$ & $1.7$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\scriptsize
\item Measured on the released seed-$42$ run, same machine for every system. For CO\textsubscript{2}, count $0.056$\,gCO\textsubscript{2} per Wh, the $56$\,gCO\textsubscript{2}/kWh grid intensity CodeCarbon applied to every run here. \rev{Every Avg is computed on unrounded values, so it can differ by one unit in the last digit from the average of the rounded cells printed here.}
\item[$\ast$] Only OPENER-Sup uses target labels. $^\ddagger$\,Qwen on a $200$-sentence subsample. $^\S$\,OWNER cost is amortised wall-clock, not a per-call latency.
\item[$\P$] A value in parentheses is left out of its row's Avg, which then uses the other three sets. \rev{The meter charges wall-clock time, and this run took $18$\,h for its $200$ sentences where its own median latency implies about $20$\,min, so a long idle stretch of the machine was billed to it as computation. Its latency, measured per sentence, is unaffected and is kept.}
\end{tablenotes}
\end{threeparttable}
\end{table}

\section{\rev{Type anchors}}
\label{sec:appendix:anchors}

\rev{\autoref{tab:anchors} gives the full anchor dictionary used by OPENER-ZS. Every entry is English, including for the Portuguese and German schemas, and all of them were fixed before any result was seen. Two entries explain most of the label-free errors of \autoref{sec:appendix:errors}. On German, \textsc{nrm} and \textsc{reg} are given near-synonymous English names, which is why the zero-shot head merges them $213$ times. On Indian-Legal, the four person types are separated by a procedural role that no anchor word can carry.}

\begin{table}[H]
\centering
\caption{\rev{The English anchor words naming each legal type.}}
\label{tab:anchors}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{threeparttable}
\rev{\begin{tabular}{@{}p{0.26\linewidth}p{0.66\linewidth}@{}}
\toprule
Type & Anchor words \\
\midrule
\multicolumn{2}{@{}l}{\emph{E-NER (en)}} \\
\textsc{business} & business, company, corporation, firm \\
\textsc{court} & court, tribunal, judiciary \\
\textsc{government} & government, agency, authority, ministry \\
\textsc{legislation/act} & legislation, act, law, statute \\
\textsc{location} & location, place, country, city \\
\textsc{person} & person, individual, name \\
\textsc{miscellaneous} & miscellaneous, other, entity, thing \\
\addlinespace[2pt]
\multicolumn{2}{@{}l}{\emph{Indian-Legal (en)}} \\
\textsc{case\_number} & case number, docket number, case no \\
\textsc{court} & court, tribunal \\
\textsc{date} & date, day, time \\
\textsc{gpe} & location, place, state, city, country \\
\textsc{judge} & judge, justice \\
\textsc{lawyer} & lawyer, attorney, counsel, advocate \\
\textsc{org} & organization, company, institution \\
\textsc{other\_person} & person, individual, name \\
\textsc{petitioner} & petitioner, plaintiff, appellant \\
\textsc{precedent} & precedent, case law, prior case, judgment \\
\textsc{provision} & provision, section, clause, article \\
\textsc{respondent} & respondent, defendant, appellee \\
\textsc{statute} & statute, act, law \\
\textsc{witness} & witness \\
\addlinespace[2pt]
\multicolumn{2}{@{}l}{\emph{LeNER-Br (pt)}} \\
\textsc{pessoa} & person, individual, name \\
\textsc{organizacao} & organization, company, institution \\
\textsc{local} & location, place, city, country \\
\textsc{tempo} & time, date, period \\
\textsc{legislacao} & legislation, law, statute, act \\
\textsc{jurisprudencia} & case law, jurisprudence, court decision, precedent \\
\addlinespace[2pt]
\multicolumn{2}{@{}l}{\emph{German-LER (de), coarse schema}} \\
\textsc{per} & person, individual, name \\
\textsc{loc} & location, place, city, country \\
\textsc{org} & organization, company, institution \\
\textsc{nrm} & legal norm, law, statute \\
\textsc{reg} & regulation, ordinance, directive \\
\textsc{rs} & case law, court decision, ruling \\
\textsc{lit} & legal literature, commentary, reference \\
\bottomrule
\end{tabular}}
\begin{tablenotes}[flushleft]
\scriptsize
\item \rev{Released with the code as \texttt{configs/legal\_anchors.yaml}. A type whose name is missing falls back to the words of the schema code itself.}
\end{tablenotes}
\end{threeparttable}
\end{table}

\section{Full evaluation setup}
\label{sec:appendix:setup}

\begin{table}[H]
\centering
\caption{Legal evaluation setup.}
\label{tab:legal-setup}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{threeparttable}
\begin{tabular}{@{}p{0.34\linewidth}p{0.60\linewidth}@{}}
\toprule
Component & Setting \\
\midrule
Entity embedder & Nomic v1.5, $D{=}768$, span-in-context, released checkpoint loaded as-is, \emph{not} retrained \citep{nussbaum2024nomic} \\
Detector (OPENER-ZS) & GLiNER-L, frozen, threshold $0.3$ \citep{zaratiana2024gliner} \\
Detector (OPENER-Sup) & GLiNER-M, frozen, threshold $0.3$, probe fit on its own train detections \\
Zero-shot typing (end-to-end) & English anchor dictionary ($1$ to $5$ words per type), each anchor embedded in $4$ short templates and averaged before $\ell_2$ normalisation, then $3$ transductive spherical $k$-means iterations and detector fusion $\beta{=}0.05$ \\
Zero-shot typing (on gold) & plain nearest prototype, single-word anchor embeddings, no transductive pass and no fusion (no detector in the loop) \\
Supervised typing & LinearSVC one-vs-rest, $C{=}1.0$, \texttt{class\_weight=balanced}, target train labels \citep{fan2008liblinear} \\
\midrule
Baselines & GLiNER-S/M/L \citep{zaratiana2024gliner}, GNER-T5-base \citep{ding2024gner}, OWNER \citep{genest2025owner}, Qwen2.5-1.5B-Instruct \citep{qwen2025} in NF4 4-bit \citep{dettmers2023qlora}, all zero-shot on the target type names \\
Test splits & first $1000$ entity-bearing sentences of each test split, Qwen on a $200$-sentence subsample \\
German-LER schema & $7$-type coarse in the tables, $19$-type fine as a stress test \\
\midrule
Quality & AMI ($\times 100$), typing-on-gold and end-to-end kept apart \\
Embedder seeds & $7$ / $42$ / $123$, full retraining per seed ($42$ released) \\
Test-set bootstrap & $K{=}1000$, mentions resampled on gold (std ${\pm}0.8$--$1.7$ AMI), sentences resampled \emph{paired} end-to-end \\
Latency / energy & p50 per sentence over warmed-up, GPU-synchronised passes, energy from CodeCarbon normalised per $1000$ sentences, released embedder. One consumer GPU (NVIDIA GTX 1660 Ti Max-Q, $6$\,GB VRAM, CUDA~12.1, PyTorch~2.5.1), same machine for every system, ${\approx}{\pm}15\%$ run-to-run (thermal throttling) \\
OWNER cost & amortised from wall-clock $t$: energy $=t\times110$\,W, latency $=t/\text{sentences}$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\scriptsize
\item Only the supervised typing head sees legal labels, every other component is a ready-to-use checkpoint. Detector variants are a free modular choice, so each OPENER head takes its best one. Baselines detect and type jointly, hence the end-to-end comparison only. Bootstrap CIs agree to ${\pm}0.1$ AMI across resampling seeds $42$ / $7$ / $123$.
\end{tablenotes}
\end{threeparttable}
\end{table}

% \autoref{tab:legal-setup} lists every component, hyperparameter and measurement convention behind the results of \autoref{sec:exp:results}. Nothing in it was tuned on legal data.